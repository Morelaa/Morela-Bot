...
class AIRich extends BaseBuilder {
    ...
    addSource(sources) {
        if (!(Array.isArray(sources) &&
            (sources.every((item) => typeof item === 'string') || sources.every((item) => Array.isArray(item) && item.every((v) => typeof v === 'string'))))) {
    ...
    addImage(imageUrl, { resolveUrl = false } = {}) {
        ...
        const list = Array.isArray(imageUrl) ? imageUrl.map(makeEntry) : [makeEntry(imageUrl)];
        this._submessages.push({
            messageType: 1,
            gridImageMetadata: {
    ...
    addVideo(videoUrl, { autoFill = true } = {}) {
        const isObjectVideo = (v) => !!v && typeof v === 'object' && !Buffer.isBuffer(v) && !!v.url;
        ...
        items.forEach((item) => {
            ...
            const file_length = isObj && item.file_length != null ? item.file_length : autoFill ? bufferPromise.then((b) => b?.length ?? 0) : 0;
            const duration = isObj && item.duration != null
                ? item.duration
                : autoFill
